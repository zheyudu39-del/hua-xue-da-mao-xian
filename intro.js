...
class CinematicIntro {
    constructor() {
        ...
        // 动画阶段定义
        this.phases = [
            { name: 'calm', duration: 3000, start: 0 },      // 0-3s: 宁静
            { name: 'rumble', duration: 2000, start: 3000 }, // 3-5s: 震动
            { name: 'avalanche', duration: 2000, start: 5000 }, // 5-7s: 雪崩
            { name: 'chase', duration: 5000, start: 7000 },  // 7-12s: 追逐
            { name: 'title', duration: 3000, start: 12000 }  // 12-15s: 标题
        ];
        this.currentPhase = 0;
        ...
        this.skier = {
            x: 800,
            y: 550,
            state: 'sitting', // sitting, alert, standing, running
            rotation: 0,
            eyeState: 'normal', // normal, wide (惊恐)
            breathCycle: 0,
            runCycle: 0
        };
        ...
        this.sun = { x: 1400, y: 150, brightness: 1.0 };
        
        // 视觉效果
        this.effects = {
            vignette: 0,
            motionBlur: 0,
            colorGrading: 'normal', // normal, desaturate, dramatic
            slowMotion: 1.0
        };
    ...
    setupEventListeners() {
        ...
        this.canvas.addEventListener('click', skipAnimation, { once: true });
    }
    
    createMountains() {
        return [
            { x: 100, y: 180, width: 350, height: 280, peaks: 3 },
            { x: 400, y: 120, width: 450, height: 350, peaks: 4 },
            { x: 800, y: 150, width: 400, height: 320, peaks: 3 },
            { x: 1150, y: 200, width: 350, height: 260, peaks: 3 }
        ];
    }
    ...
    createTrees() {
        ...
        return trees.sort((a, b) => a.y - b.y); // 按Y排序实现景深
    }
